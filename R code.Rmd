...
# Filtering the IDB master data file to be compatible to the OECD database
...
#OECD data
#Renaming the variable names for clarity
...
#Merging the OECD and IDB databases
data_longer<-rbind(PSCT_oecd_longer,idb_clean_longer)%>%pivot_wider(names_from = sct,values_from = value)%>%select(Country,Commodity,Year,`Level of production`,`Value of production (at farm gate)`,`Support based on commodity outputs`,`Market price support`,`Payments based on output`,`Payments based on input use`,`Variable input use`,`Fixed capital formation`,`On-farm services`,`Payments based on current A/An/R/I, production required, single commodity`,`Payments based on non-current A/An/R/I, production required`,PSCT,`%PSCT`)%>%ungroup()

rep_dom<-data_longer%>%filter(Country=="Dominican Republic")

#write_xlsx(data_longer,"~/Desktop/Analytical work PSE obesity/Data/data_OECD_AGRIMON.xlsx")

blog<-data_longer%>%filter(Commodity=="Sugar")%>%filter(Year=="2016"|Year=="2017"|Year=="2018"|Year=="2019")%>%select(Country,Year,Commodity,`%PSCT`)


#Making aggregate "MEAT variable" to be compatible with GDD data
#Making dataset combining beef,sheep and poultry
#Important modifications:
#Inferred GFR from %PSCT and PSCT
...
#Creating data set with every country and variables mentioned in presentation
##1 How many countries for which do we have estimates on for our commodities
...
#Finding relationship between total subsidies and Ag subsidies
...
pse_subsidy<-merge(subsidies,supp,by=c("Country","Year"))

#data without country or time dimension for the correlation coefficient
pse_subs<-pse_subsidy%>%mutate(`%PSE(total spending)`=(`Producer Support Estimate (PSE)`*100)/tot_spending)%>%mutate(`%TSE(total spending)`=(`Total Support Estimate (TSE)`)/tot_spending)%>%select(`subsidies (%)`,`%PSE(total spending)`,`%TSE(total spending)`)


pse_subsidy_1<-pse_subsidy%>%filter(!(Country=="Bolivia"|Country=="Panama"))%>%mutate(`%PSE(total spending)`=(`Producer Support Estimate (PSE)`*100)/tot_spending)%>%mutate(`%TSE(total spending)`=(`Total Support Estimate (TSE)`*100)/tot_spending)%>%group_by(Country)%>%mutate(correlation_dol=cor(`subsidies ($)`,`Producer Support Estimate (PSE)`))%>%mutate(correlation_pse=cor(`%PSE(total spending)`,`subsidies (%)`))%>%mutate(correlation_tse=cor(`%TSE(total spending)`,`subsidies (%)`))%>%mutate(p_val_pse=cor.test(`%PSE(total spending)`,`subsidies (%)`,use = "complete.obs" )$p.value)%>%mutate(p_val_tse=cor.test(`%TSE(total spending)`,`subsidies (%)`,use = "complete.obs" )$p.value)
pse_test<-pse_subsidy_1 %>%select(Country,10:14)%>%mutate(p_val_pse=round(p_val_pse,digits=3))%>%mutate(p_val_tse=round(p_val_tse,digits=3))%>%unique()

corr <- round(cor(pse_subs,use = "complete.obs" ), digits=3)
cor_tab<-tidy(head(corr[, 1:3]))
...
p_val<-tidy(head(round(corrp.mat[, 1:3],digits=4)))

#Correlogram
ggcorrplot(corr, hc.order =TRUE, 
           type ="lower", p.mat = corrp.mat, insig="blank", method="circle", lab= TRUE,title="Correlogram of PSE,TSE, Susbidies", 
           ggtheme=theme_bw)
...
#Note: After using this algorithm, it is necessary to detach(plyr) or else the code does not run due to conflicts with plyr
...
#WTO data +Subsidies data  to reproduce findings from Abay
